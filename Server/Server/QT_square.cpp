...
void QT_square::setused(bool input)
{
	used = input;
}

bool QT_square::isused()
{
	return used;
}
...
void QT_square::activate() //activate child_nodes
{
	for(int i = 0; i < 4; i++)
	{
		child_node[i]->setused(true);
	}
}

void QT_square::insert(Circle *c)
{
	if(child_node[0] != NULL && child_node[0]->isused())
	{
	...
	if(Cs.size()> Max_objects && tree_lvl < max_tree_lvl)
	{
		if(child_node[0] != NULL && !child_node[0]->isused())
		{
			activate();
...
int QT_square::output(string* output)
{
	int size = 0;
	stringstream ss;
	ss<<bounding_box.min.GetX();
	output->append(ss.str());
	output->append(" ");
	ss.str("");
	ss<<bounding_box.min.GetY();
	output->append(ss.str());
	output->append(" ");
	ss.str("");
	ss<<bounding_box.max.GetX();
	output->append(ss.str());
	output->append(" ");
	ss.str("");
	ss<<bounding_box.max.GetY();
	output->append(ss.str());
	output->append(" ");
	size++;
	if(child_node[0] != NULL && child_node[0]->isused())
	{
		for(int i = 0; i < 4; i++)
		{
			size += child_node[i]->output(output);
		}
	}
	return size;
}